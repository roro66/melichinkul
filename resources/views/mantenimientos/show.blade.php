...
<div class="space-y-6">
    @php
        $maintenance = \App\Models\Mantenimiento::with(['vehiculo', 'tecnicoResponsable', 'conductorAsignado'])->findOrFail($id);
    @endphp
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ $maintenance->vehiculo->patente }} - {{ ucfirst($maintenance->tipo) }}
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->vehiculo->patente }} - {{ $maintenance->vehiculo->marca }} {{ $maintenance->vehiculo->modelo }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Tipo:</span>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                    {{ $maintenance->tipo === 'preventivo' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' : 
                       ($maintenance->tipo === 'correctivo' ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300' : 
            ...
            <div>
                ...
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                    {{ $maintenance->estado === 'completado' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 
                       ...
                       ($maintenance->estado === 'cancelado' ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' : 
            ...
            <div>
                ...
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->fecha_programada?->format('d/m/Y') ?? 'Sin fecha' }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Fecha de Inicio:</span>
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->fecha_inicio?->format('d/m/Y') ?? 'Sin fecha' }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Fecha de Fin:</span>
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->fecha_fin?->format('d/m/Y') ?? 'Sin fecha' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <p class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{{ $maintenance->descripcion_trabajo }}</p>
        
        @if($maintenance->motivo_ingreso)
            <div class="mt-4">
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Motivo de Ingreso:</span>
                <p class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{{ $maintenance->motivo_ingreso }}</p>
            </div>
        @endif

        @if($maintenance->trabajos_realizados)
            <div class="mt-4">
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Trabajos Realizados:</span>
                <p class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{{ $maintenance->trabajos_realizados }}</p>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <p class="text-sm font-medium text-gray-900 dark:text-white">${{ number_format($maintenance->costo_repuestos, 0, ',', '.') }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Costo Mano de Obra:</span>
                <p class="text-sm font-medium text-gray-900 dark:text-white">${{ number_format($maintenance->costo_mano_obra, 0, ',', '.') }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Costo Total:</span>
                <p class="text-lg font-bold text-gray-900 dark:text-white">${{ number_format($maintenance->costo_total, 0, ',', '.') }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Horas Trabajadas:</span>
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->horas_trabajadas ?? 'N/A' }} hrs</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->tecnicoResponsable->name ?? 'Sin asignar' }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Conductor Asignado:</span>
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->conductorAsignado->nombre_completo ?? 'Sin asignar' }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Taller/Proveedor:</span>
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->taller_proveedor ?? 'N/A' }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Kilometraje:</span>
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->kilometraje_en_mantenimiento ? number_format($maintenance->kilometraje_en_mantenimiento, 0, ',', '.') . ' km' : 'N/A' }}</p>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Horómetro:</span>
                <p class="text-sm text-gray-900 dark:text-white">{{ $maintenance->horometro_en_mantenimiento ? number_format($maintenance->horometro_en_mantenimiento, 0, ',', '.') . ' hrs' : 'N/A' }}</p>
            </div>
            @if($maintenance->observaciones)
                <div class="md:col-span-2">
                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Observaciones:</span>
                    <p class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{{ $maintenance->observaciones }}</p>
                </div>
            @endif
        </div>
    </div>
