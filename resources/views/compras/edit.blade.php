...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Editar compra #{{ $purchase->id }}</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Proveedor: {{ $purchase->supplier?->name }}</p>
        </div>
        <a href="{{ route('compras.show', $purchase->id) }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Volver</a>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('compras.update', $purchase->id) }}" method="POST" id="purchase-form" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="reference" name="reference" value="{{ old('reference', $purchase->reference) }}" maxlength="64"
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="2" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">{{ old('notes', $purchase->notes) }}</textarea>
            </div>
                ...
                <div class="overflow-x-auto border border-gray-200 dark:border-gray-600 rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                        ...
                        <tbody id="items-tbody" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                            @php
                                $oldItems = old('items');
                                $itemsSource = $oldItems !== null ? $oldItems : $purchase->items->map(fn($i) => [
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                <a href="{{ route('compras.show', $purchase->id) }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Cancelar</a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (initialItems && initialItems.length > 0) {
        initialItems.forEach((item, i) => addRow(i, item));
    } else {
        addRow(0);
    }
});
