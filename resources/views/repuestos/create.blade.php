...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Nuevo repuesto</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Agregar al catálogo de inventario</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('repuestos.store') }}" method="POST" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code') }}" required maxlength="64"
                ...
                <div>
                    ...
                    <select id="category" name="category" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                        @foreach($categories as $value => $label)
                            <option value="{{ $value }}" {{ old('category', 'spare_part') === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="brand" name="brand" value="{{ old('brand') }}" maxlength="255"
                ...
                <div>
                    ...
                    <input type="number" id="reference_price" name="reference_price" value="{{ old('reference_price') }}" min="0"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                </div>
            </div>
            <div class="flex items-center gap-4">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="has_expiration" value="1" {{ old('has_expiration') ? 'checked' : '' }}
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white rounded-lg">Crear repuesto</button>
            </div>
        </form>
    </div>
</div>
